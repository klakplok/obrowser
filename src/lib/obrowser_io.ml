...
external dom_of_xml : string -> Obrowser_jsoo.obj = "caml_js_dom_of_xml"
external pretty_xml_of_dom : Obrowser_jsoo.obj -> string = "caml_js_pretty_xml_of_dom"
external xml_of_dom : Obrowser_jsoo.obj -> string = "caml_js_xml_of_dom"
...
module Node = struct
  ...
  let get_attribute node name =
    try node  >>> call_method "getAttribute" [| string name |] >>> as_string with _ -> failwith "get_attribute"
...
module Html = struct
  ...
  let map_table ?style ?(attrs = []) ?tr_style ?(tr_attrs = []) ?td_style ?(td_attrs = []) ?(f = (fun _ _ c -> c)) t =
    ...
    (fun (n, v) -> 
       Node.append f (Html.create "input"
                        ~attrs:[("type", "text"); 
                                ...
                                ("value", v)] ()))
    params ;
  ignore (Obrowser_jsoo.call_method "submit" [||] f)

let window = Obrowser_jsoo.eval "window"

let redirect_get url = Obrowser_jsoo.set "location" (Obrowser_jsoo.string url) window
