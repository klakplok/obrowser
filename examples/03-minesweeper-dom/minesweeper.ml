...
type demin_cf = 
    { bd : cell array array;
      dom : Obrowser_io.Node.t array array;
      cf : config ;
...
let draw_cell dom bd =
  Obrowser_io.Node.set_attribute dom "src"
...
let disable_events d =
  for y = 0 to d.cf.nbrows - 1 do
    for x = 0 to d.cf.nbcols - 1 do
      Obrowser_io.Node.register_event d.dom.(y).(x) "onclick" (fun () -> Obrowser_io.alert "GAME OVER") ()
    done 
  done
...
let _ =
  let d = create_demin 40 30 50 in
  let board_div = Obrowser_io.Node.get_element_by_id Obrowser_io.Node.document "main" in
  let mode = ref Normal in
    Obrowser_io.Node.append board_div (Obrowser_io.Node.text "Mode : ") ;
    let img = Obrowser_io.Node.element "img" in
      Obrowser_io.Node.append board_div img ;
      Obrowser_io.Node.set_attribute img "src" "sprites/bomb.png" ;
      Obrowser_io.Node.register_event
	img "onclick"
	(fun () ->
	   match !mode with
	     | Normal -> mode := Flag ; Obrowser_io.Node.set_attribute img "src" "sprites/flag.png"
	     | Flag -> mode := Normal ; Obrowser_io.Node.set_attribute img "src" "sprites/bomb.png"
	) () ;
      Obrowser_io.Node.append board_div (Obrowser_io.Node.element "br") ;
      Obrowser_io.Node.append board_div (Obrowser_io.Node.element "br") ;
      let div = Obrowser_io.Node.element "div" in
      Obrowser_io.Node.set_attribute div "style" "line-height: 0;" ;
      let buf = Obrowser_io.Fragment.create () in
      for y = 0 to d.cf.nbrows - 1 do
	let imgs = ref [] in
	  for x = 0 to d.cf.nbcols - 1 do
	    let img = Obrowser_io.Node.element "img" in
	      imgs := img :: !imgs ;
	      Obrowser_io.Node.set_attribute img "src" "sprites/normal.png" ;
	      Obrowser_io.Node.register_event
		img "onclick"
		(fun () ->
		   match !mode with
			 ...
			 if d.bd.(x).(y).seen then ()
			   ...
			   draw_board d ;
			   disable_events d ;
			   Obrowser_io.alert "YOU LOSE"
			 ) else reveal d x y
		     | Flag ->
			 d.bd.(x).(y).flag <- not d.bd.(x).(y).flag ;
			 draw_cell img d.bd.(x).(y)
		) () ;
	      Obrowser_io.Fragment.append buf img
	  done ;
	  Obrowser_io.Fragment.append buf (Obrowser_io.Node.element "br") ;
	  d.dom.(y) <- Array.of_list (List.rev !imgs)
      done ;
      Obrowser_io.Fragment.flush div buf ;
      Obrowser_io.Node.append board_div div
